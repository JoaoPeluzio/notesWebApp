...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
    ...
    function  handleStartRecording() {
      ...
      if (!isSpeechRecognitionAPIAvaiable) {
        alert('Desculpe! O seu navegador não suporta a API de gravação!')
        return
      }
  ...
  return (
      <Dialog.Root>
            ...
            <Dialog.Trigger className='rounded-md flex flex-col bg-slate-700 p-5 gap-3 text-left overflow-hidden relative hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400 outline-none'>
                ...
                <span className='tent-sm font-medium text-slate-200'>
                  Adicionar nota
                </span>
              ...
              <div className='absolute bottom-0 left-0 right-0 h-1/2 bg-gradient-to-t from-slate-500/60 to-slate-100/0 pointer-events-none' /> 
            ...
            <Dialog.Content className='overflow-hidden fixed inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[440px] w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col outline-none'>


                  <Dialog.Close className='absolute right-0 top-0 bg-slate-800 p-1.5 text-slate-400 hover:text-slate-100 rounded-bl-lg '>
